...
def predict():
    ...
    if not file or file.filename == '':
        return render_template("index.html", results="<p>No file uploaded.</p>")
    ...
    with torch.no_grad():
        ...
        seg_mask_pil = Image.fromarray(seg_mask.astype(np.uint8))
        seg_tensor = transforms.ToTensor()(seg_mask_pil.resize((512, 512), Image.NEAREST)).to(device)

    seg_mask_rgb = convert_seg_to_rgb(seg_mask)
    seg_rgb_path = os.path.join(STATIC_FOLDER, f"seg_rgb_{base_name}.png")
    seg_mask_rgb.save(seg_rgb_path)

    overlay_img = overlay_mask_on_image(image_pil, seg_mask_rgb)
    ...
    <ul class="legend">
        ...
        <li><span style="color:blue;">■</span> Non-Tumor Region</li>
    </ul>
    '''
